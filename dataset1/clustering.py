...
# Visualize data
plt.scatter(X['radius_worst'], X['perimeter_worst'], marker='+', c=y)
...
plt.plot(k, sc_scores_km, '-o', label='K-means')
...
plt.plot(k, time_km, '-o', label='K-means')
...
# Visualize clusters
# labels
plt.scatter(X_train[:, 20], X_train[:, 22], marker='+', c=y_train)
plt.xlabel('radius_worst')
plt.ylabel('perimeter_worst')
...
# Cross tabulation
df = pd.DataFrame({'labels': labels, 'diagnosis': y_train})
ct = pd.crosstab(df['labels'], df[ 'diagnosis'])
print(ct)
# Visualize clusters and cluster centers
centroids = kmeans.cluster_centers_
centroids_x = centroids[:, 20]
centroids_y = centroids[:, 22]
plt.scatter(X_train[:, 20], X_train[:, 22], marker='+', c=labels)
